...
class SensorDataGenerator(object):
    ...
    def __init__(
        self,
        epochOffsetSeconds: float = 0.0,
        useCurrentTime: bool = True,
        alignGeneratorToDay: bool = True,
    ):
        """
        Constructor.
    ...
    def generateDailyEnvironmentHumidityDataSet(
        self,
        noiseLevel: int = DEFAULT_NOISE,
        minValue: float = MIN_ENV_HUMIDITY,
        maxValue: float = MAX_ENV_HUMIDITY,
        ...
        """
        if maxValue < self.MIN_ENV_HUMIDITY or maxValue > self.MAX_ENV_HUMIDITY:
            maxValue = self.MAX_ENV_HUMIDITY
        if minValue < self.MIN_ENV_HUMIDITY or minValue >= maxValue:
            minValue = maxValue - 1

        return self.generateDailySensorDataSet(
            curveType=self.DEFAULT_HUMIDITY_CURVE,
    ...
    def generateDailyEnvironmentPressureDataSet(
        self,
        noiseLevel: int = DEFAULT_NOISE,
        minValue: float = MIN_ENV_PRESSURE,
        maxValue: float = MAX_ENV_PRESSURE,
        useSeconds: bool = False,
    ):
        """
        Generates a time-series data set for indoor temperature simulation over a 24-hour period.
        ...
        values for those time entries.
        """
        if maxValue < self.MIN_ENV_PRESSURE or maxValue > self.MAX_ENV_PRESSURE:
            maxValue = self.MAX_ENV_PRESSURE
        if minValue < self.MIN_ENV_PRESSURE or minValue >= maxValue:
            minValue = maxValue - 1

        return self.generateDailySensorDataSet(
            curveType=self.DEFAULT_PRESSURE_CURVE,
            noiseLevel=noiseLevel,
            minValue=minValue,
            maxValue=maxValue,
            startHour=0,
    ...
    def generateDailyIndoorTemperatureDataSet(
        self,
        noiseLevel: int = DEFAULT_NOISE,
        minValue: float = MIN_INDOOR_TEMP,
        maxValue: float = MAX_INDOOR_TEMP,
        useSeconds: bool = False,
    ):
        """
        Generates a time-series data set for indoor temperature simulation over a 24-hour period.
    ...
    def generateDailyMonitorTemperatureDataSet(
        self,
        noiseLevel: int = DEFAULT_NOISE,
        ...
        values for those time entries.
        """
        if maxValue < self.MIN_MONITOR_TEMP or maxValue > self.MAX_MONITOR_TEMP:
            maxValue = self.MAX_MONITOR_TEMP
        if minValue < self.MIN_MONITOR_TEMP or minValue >= maxValue:
            minValue = maxValue - 1
    ...
    def generateDailySensorDataSet(
        self,
        curveType: int = FULL_WAVE,
        noiseLevel: int = DEFAULT_NOISE,
        minValue: float = DEFAULT_MIN_VALUE,
        maxValue: float = DEFAULT_MAX_VALUE,
        startHour: int = MIN_HOURS,
        endHour: int = MAX_HOURS,
        useSeconds=False,
    ):
        """
        ...
        if noiseLevel < self.NO_NOISE:
            noiseLevel = self.NO_NOISE
        if noiseLevel > self.MAX_NOISE:
            noiseLevel = self.MAX_NOISE
        ...
        if startHour < self.MIN_HOURS or minValue > self.MAX_HOURS:
            startHour = self.MIN_HOURS
        if endHour < 0 or endHour > self.MAX_HOURS:
            endHour = self.MAX_HOURS

        # calc total data points to be generated
        totalDataPoints = (endHour - startHour) * 60

        if useSeconds:
            totalDataPoints = totalDataPoints * 60
        if totalDataPoints == 0:
            totalDataPoints = 1

        # create evenly spaced number of 'totalDataPoints' between 'startHour' and 'endHour'
        timeEntries = calcLib.linspace(
            start=startHour, stop=endHour, num=totalDataPoints
        )
        ...
        scaledValuesClean = calcLib.interp(
            dataValuesClean,
            (dataValuesClean.min(), dataValuesClean.max()),
            (minValue, maxValue),
        )
        dataSet = SensorDataSet(
            epochOffsetSeconds=self.epochOffsetSeconds,
            useCurrentTime=self.useCurrentTime,
        ...
        if noiseLevel != self.NO_NOISE:
            ...
            logging.debug(
                "Noise=%f; Noise Scale=%f; Mean Magnitude=%f"
                % (noiseLevel, noiseScale, meanMag)
            )
    ...
    def generateOnScreenGraph(
        self,
        dataSet=None,
        chartTitle: str = "Sample Data",
        chartXLabel: str = "X Axis",
        chartYLabel: str = "Y Axis",
    ):
        """
...
class SensorDataSet:
    ...
    def __init__(
        ...
        if not useCurrentTime:
            try:
                ...
                logging.warning(
                    "Offset seconds since epoch not a float. Using current time instead."
                )
    ...
    def setTimeEntries(self, timeEntries):
        ...
        if not timeEntries is None:
            ...
            logging.info(
                "timeEntries tuple. Array Size: %s  ND Size: %s  Dimensions: %s  Shape: %s  Type: %s",
                self.timeEntries.size,
                timeEntries.size,
                timeEntries.ndim,
                timeEntries.shape,
                timeEntries.dtype,
    ...
    def setDataEntries(self, dataEntries):
        ...
        if not dataEntries is None:
            ...
            logging.info(
                "dataEntries tuple. Array Size: %s  ND Size: %s  Dimensions: %s  Shape: %s  Type: %s",
                self.dataEntries.size,
                dataEntries.size,
                dataEntries.ndim,
                dataEntries.shape,
                dataEntries.dtype,
...
def main():
    ...
    sensorDataGeneratorEpoch = SensorDataGenerator(
        useCurrentTime=False, alignGeneratorToDay=True
    )

    # run generic example - 1 second samples over 24 hours - start time will be this system's Epoch
    sensorDataSet = sensorDataGeneratorEpoch.generateDailySensorDataSet(
        noiseLevel=10,
        startHour=0,
        endHour=24,
        minValue=10,
    ...
    sensorDataSet = sensorDataGenerator.generateDailyIndoorTemperatureDataSet(
        noiseLevel=15,
    ...
    sensorDataSet = sensorDataGenerator.generateDailyEnvironmentHumidityDataSet(
        noiseLevel=10,
