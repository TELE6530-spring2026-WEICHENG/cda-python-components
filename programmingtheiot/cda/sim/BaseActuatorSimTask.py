...
class BaseActuatorSimTask:
    ...
    def __init__(
        ...
        ### Instantiate ActuatorData object and set as response
    ...
    def updateActuator(self, data: ActuatorData) -> ActuatorData:
        ...
        """
        logging.debug(
            "type check: self.typeID=%s, data.typeID=%s",
            str(self.typeID),
            str(data.getTypeID()),
        )
        ...
        if data and self.typeID == data.getTypeID():
            ...
            if (
                ...
                logging.debug(
                    "New actuator command and value to be applied: Current command: %s, Current value: %s",
                    str(curCommand),
                    str(curVal),
                )
                if curCommand == ConfigConst.COMMAND_ON:
                    ...
                    logging.warning(
                        "ActuatorData command is unknown. Ignoring: %s", str(curCommand)
                    )
                    statusCode = -1
